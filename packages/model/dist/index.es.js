...
var Hasher = class {
  ...
  hash(value) {
    this.value = hashSync(value, this.saltOrRounds || 10);
  }
  toString() {
    return this.value;
  }
  compare(valueToCompare) {
    return compareSync(valueToCompare, this.value);
  }
  hashCode(str) {
    let hash = 0, i, chr;
    if (str.length === 0)
...
var hashType = {
  onCreate: ({ value }) => {
    if (typeof value === "string") {
      const salt = config ? config.getConfig("APP_KEY", void 0) : void 0;
...
var Model = class {
  ...
  static query() {
    return query().table(this.$table).parseResult((result) => {
  ...
  query() {
    return query().table(this.constructor.$table).parseResult((result) => {
      if (Array.isArray(result)) {
        return result.map((r) => {
  ...
  async save() {
    ...
    if (this.$databaseInitialized) {
      await query().table($table).where($primary, fields[$primary]).update(fields);
      id = fields[$primary];
    } else {
  ...
  async delete() {
    const { $table, $primary } = this.constructor;
    if (this.$databaseInitialized) {
      await query().table($table).where($primary, this.$values[$primary]).delete();
    }
    this.$databaseInitialized = false;
  }
  fill(fields) {
...
export { Field2 as Field, Hasher, Model, ModelDecorator_default as Table, getModels, types_default as typeManager };
