 ...
 **/import { CustomException, exceptionLog } from '@acai/utils';
...
var AdapterNotFound = class extends CustomException {
...
var PortOccupied = class extends CustomException {
...
var RouteNotFound = class extends CustomException {
...
async function smartResponse(payload, request, viewPrefix) {
  ...
  if (typeof payload[0] === "function" && payload[0].utility === "response") {
    ...
    if (data.view) {
      body = readFileSync(join(`${process.cwd()}`, viewPrefix || "", data.view), {
        encoding: "utf-8"
      });
    }
  } else {
...
var HttpAdapter = class {
  ...
  getMatch(path3, req) {
    const match = router(path3, req.method, route.build(false));
    if (!match)
      return;
...
var ControllerNotFoundException = class extends CustomException {
...
async function findController(controllerPath, route2) {
  ...
  const pathString = await findFile(join(process.cwd(), controller)) || controller;
    ...
    if (response2 === void 0) {
      if (error.shouldReport !== false && process.env.testing !== "true") {
        if (error.report)
          ...
          exceptionLog(error.message, error.stack?.split("\n"), error.data);
      }
    }
    if (error.critical) {
...
var MiddlewareNotFound = class extends CustomException {
...
export { HttpAdapter, src_default as default };
