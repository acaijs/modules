...
function calculateConstraints(oldColumns, updatedColumns) {
    ...
    Object.keys(updatedColumns).forEach(key => {
        var _a;
        if (updatedColumns[key] && updatedColumns[key].foreign && oldColumns[key] && oldColumns[key].foreign && !isEquals_1.default(updatedColumns[key].foreign, oldColumns[key].foreign)) {
            const column = updatedColumns[key];
            const foreign = column.foreign;
            const response = [];
            // remove previous
