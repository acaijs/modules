...
var findMethod = async (regex) => {
  await new Promise((r) => {
    glob.glob(regex || "./**/*.{test,tests}.{ts,js}", {
      ...
      Promise.all(matches.map(async (file) => (function (t) { return Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(t)); }); })(path.join(process.cwd(), file)))).then(r);
    });
  });
};
