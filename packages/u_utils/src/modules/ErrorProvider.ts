// Packages
import { ProviderInterface, RequestInterface, ServerInterface, CustomExceptionInterface } from "@acai/interfaces";
...
export default class ErrorProvider {
	...
	private genericError (error: CustomExceptionInterface & {[key in string]: any}, request: RequestInterface) {
		// arrange data
		const data = {
	...
	private sqlError (error: CustomExceptionInterface & {[key in string]: any}) {
		if (error.sqlMessage) {
			const data = {
				query	: error.query,
